/* ============================= DATA LAYER ============================= */
const DB_KEY = "vsprout_words_v1",
  SET_KEY = "vsprout_settings_v1",
  LOG_KEY = "vsprout_log_v1",
  TOPIC_ACT_KEY = "vsprout_topic_activity_v1";
/* ============================================================================
   VOCABULARY DATA — schema & how to add more topics
   ----------------------------------------------------------------------------
   Every word entry is a plain object with these fields:

     word          required. English headword (lowercase, as typed by users).
     ipa           IPA pronunciation. Convention: American English throughout
                   (e.g. /ɑː/ not /ɒ/) so every card sounds consistent.
     pos           ONE value only: "noun" | "verb" | "adj" | "adv" | "phrase".
                   Never combine two (no "noun/verb") — pick the sense used in
                   `example`. If a word is genuinely common as two different
                   parts of speech, add it as two separate entries instead.
     meaning       required. Vietnamese meaning.
     example       Natural English sentence using the word.
     exampleVi     Accurate, natural Vietnamese translation of `example`.
     collocations  Comma-separated string, 1-3 common collocations.
                   Leave "" if nothing notably idiomatic exists — don't force it.
     synonyms      Comma-separated close synonyms. Leave "" rather than
                   including a loose/inaccurate match just to fill the field.
     antonyms      Comma-separated antonyms. Same rule as synonyms.
     topic         Must exactly match the topic name used across all entries
                   in that set — this string is what groups words together
                   in the "Học" tab topic picker and the topic chip in the UI.
     cefr          Optional. One of "A1"|"A2"|"B1"|"B2"|"C1"|"C2". CEFR level
                   of the word, independent of the app's own FSRS difficulty
                   (FSRS difficulty is learned per-user from review history;
                   cefr is a fixed, objective reference level). Leave "" if
                   unknown rather than guessing.
     note          Optional. Short usage note in Vietnamese — things a
                   dictionary entry alone doesn't convey: register (formal/
                   informal), common confusion with another word, a grammar
                   quirk (countable/uncountable, irregular plural), etc.
                   Leave "" unless there's something genuinely worth flagging.
                   (Pronunciation playback itself needs no data field — the
                   speak() function already handles that via TTS.)

   HOW TO ADD A NEW TOPIC (e.g. "Du lịch"):
     1. Copy TOPIC_FINANCE_WORDS below as a template, rename to e.g.
        TOPIC_TRAVEL_WORDS, fill in entries following the schema above.
     2. Add `...TOPIC_TRAVEL_WORDS,` to the ALL_TOPIC_WORDS array at the
        bottom of this block.
     That's it — the topic picker, first-run seeding, and everything else
     read from ALL_TOPIC_WORDS automatically, no other code changes needed.
   ----------------------------------------------------------------------------
   VOCABULARY ROADMAP (not yet built — reference for future expansion)
   ----------------------------------------------------------------------------
   Target: ~3,500 words total, sized for TOEIC 500-650 (CEFR B1) coverage.
   Build order = priority order below. Each row becomes one TOPIC_..._WORDS
   array once written; "Tài chính - Ngân hàng" (40 words) is the only one
   built so far and is a partial start on priority #8.

     # | Chủ đề (topic)             | Số từ | Nội dung
     --+----------------------------+-------+---------------------------------
     1 | Work & Office              |  400  | Công việc, văn phòng, nhân viên,
       |                            |       | tài liệu, lịch làm việc, thiết bị
     2 | Daily Life                 |  400  | Hoạt động hằng ngày, thời gian,
       |                            |       | gia đình, sinh hoạt, mua bán
     3 | Communication              |  250  | Gọi điện, email, nhắn tin, hỏi đáp
     4 | Business                   |  300  | Công ty, kinh doanh, khách hàng,
       |                            |       | hợp đồng, dự án, quản lý
     5 | Travel & Transportation    |  250  | Sân bay, khách sạn, tàu xe, vé
     6 | Shopping                   |  200  | Mua sắm, giá cả, hóa đơn, giảm giá
     7 | Meetings & Presentations   |  200  | Họp, báo cáo, thuyết trình
     8 | Finance & Banking          |  250  | (40/250 đã có — xem TOPIC_FINANCE_WORDS)
     9 | Customer Service           |  150  | Hỗ trợ khách hàng, khiếu nại, bảo hành
    10 | Education                  |  150  | Trường học, khóa học, bài kiểm tra
    11 | Food & Drink                |  150  | Đồ ăn, đồ uống, nhà hàng, thực đơn
    12 | Health                      |  150  | Cơ thể, bệnh, thuốc, khám chữa bệnh
    13 | Technology                  |  150  | Máy tính, điện thoại, Internet
    14 | Housing                     |  100  | Nhà ở, nội thất, thuê nhà, sửa chữa
    15 | Entertainment               |  100  | Phim, nhạc, sở thích, sự kiện
    16 | Sports                      |  100  | Thể thao, tập luyện, thi đấu
    17 | Environment                 |  100  | Thời tiết, môi trường, năng lượng
    18 | Marketing                   |  100  | Quảng cáo, thương hiệu, bán hàng
    19 | Law & Government            |   50  | Pháp luật, cơ quan nhà nước, quy định

   When actually writing these, follow the schema above exactly (esp. the
   "don't force synonyms/collocations/cefr" rule) — accuracy over completeness.
   ============================================================================ */
...
function fsrsOpts() {
  return {
    retention: settings.desiredRetention || 0.9,
    fuzz: settings.enableFuzz !== false,
  };
}
...
function loadData() {
  try {
    words = JSON.parse(localStorage.getItem(DB_KEY)) || null;
  } catch (e) {
    words = null;
  }
  if (!words) {
    ...
    const missing = ALL_TOPIC_WORDS.filter(
  ...
  try {
    topicActivity = JSON.parse(localStorage.getItem(TOPIC_ACT_KEY)) || {};
  } catch (e) {}
}
function saveWords() {
...
function makeCard(w) {
  return {
    ...
    collocations: toChipArr(w.collocations),
    synonyms: toChipArr(w.synonyms),
    antonyms: toChipArr(w.antonyms),
    topic: w.topic || "Chung",
    cefr: w.cefr || "",
    note: w.note || "",
    state: "new",
...
function logEvent(type) {
  const k = todayKey();
  if (!dailyLog[k])
    dailyLog[k] = { reviews: 0, learned: 0, correct: 0, total: 0 };
  if (type === "review") {
    dailyLog[k].reviews++;
    dailyLog[k].total++;
  }
...
function logStudyActivity(grade) {
  ...
  if (lastActionTime > 0) {
    ...
    if (gap > 0 && gap < 120000)
      dailyLog[k].timeMs = (dailyLog[k].timeMs || 0) + gap; // ignore long idle gaps
  }
  lastActionTime = now;
  saveLog();
}
...
document
  .querySelectorAll(".nav-btn")
  .forEach((b) => b.addEventListener("click", () => goTab(b.dataset.tab)));
...
function weakScore(w) {
  ...
  let score = 0;
  score += wrong * 15;
  score += Math.max(0, 0.7 - accuracy) * 100;
...
function priorityTier(w) {
  ...
  if (
    w.lastReview &&
    w.state !== "new" &&
    Date.now() - w.lastReview < 2 * 86400000
  )
    return 1; // vừa học/xem gần đây
...
function overdueOrder(list) {
  ...
  return Object.keys(buckets)
    .sort((a, b) => a - b)
    .flatMap((k) => shuffleArr(buckets[k]));
}
...
function renderHome() {
  ...
  document.getElementById("homeNewProgress").textContent =
    `${log.learned || 0}/${goal}`;
  ...
  const pct =
    totalGoal > 0
      ? Math.min(100, Math.round((doneToday / totalGoal) * 100))
      : 100;
...
function renderHomeDecks() {
  ...
  block.style.display = "";
  document.getElementById("homeDeckList").innerHTML = list
    .map((t) => {
...
function cardBackHTML(w) {
  ...
  if (
    ...
    settings.showCollocations !== false
  ) {
...
function topicStatsFor(topicWords) {
  ...
  const dueCount = topicWords.filter(
    (w) => w.state !== "new" && w.due <= Date.now(),
  ).length;
...
function topicCefrLabel(topicWords) {
  ...
  topicWords.forEach((w) => {
    if (w.cefr) counts[w.cefr] = (counts[w.cefr] || 0) + 1;
  });
...
function renderTopicList() {
  ...
  list.innerHTML = names
    .map((t) => {
      ...
      let badges = [
        stat.newCount > 0
          ? `<span class="topic-badge new">🟢 ${stat.newCount} mới</span>`
          : "",
        stat.dueCount > 0
          ? `<span class="topic-badge due">🟠 ${stat.dueCount} ôn</span>`
          : "",
      ].join("");
...
let learnCompletedIds = new Set(),
  learnOriginalTotal = 0;
function startLearnTopic(topic) {
  ...
  learnCompletedIds = new Set();
  learnOriginalTotal = learnQueue.length;
...
let reviewCompletedIds = new Set(),
  reviewOriginalTotal = 0;
...
function startTopicReview(topic) {
  goTab("review");
  let due = overdueOrder(
    getDueWords().filter((w) => (w.topic || "Chung") === topic),
  );
  ...
  reviewFlipped = false;
  lastActionTime = Date.now();
  reviewCompletedIds = new Set();
  reviewOriginalTotal = reviewQueue.length;
  document.getElementById("reviewCount").textContent =
    `0/${reviewOriginalTotal}`;
...
function flipLearn() {
  ...
  document
    .getElementById("learnCard")
    .classList.toggle("flipped", learnFlipped);
  const btnIds = [
    "learnAgainBtn",
    "learnHardBtn",
    "learnGoodBtn",
  ...
  btnIds.forEach(
    (id) => (document.getElementById(id).disabled = !learnFlipped),
  );
  document.getElementById("learnRateRow").style.display = learnFlipped
    ? "grid"
    : "none";
...
function learnRate(grade) {
  ...
  if (grade === 1) {
    ...
    const insertAt = Math.min(
      learnQueue.length,
      learnIdx + 1 + Math.min(3, learnQueue.length - learnIdx - 1),
    );
...
function renderReviewModeList() {
  ...
  const items = [
    {
      emoji: "🎯",
      name: "Từ yếu",
      sub: "Chưa nhớ ổn định, dễ quên · xếp theo mức yếu",
      count: weak.length,
      action: "startWeakPractice()",
    },
    {
      emoji: "⚡",
      name: "Luyện tập nhanh",
      sub: "Trắc nghiệm ngẫu nhiên · không ảnh hưởng lịch ôn",
      count: learnedCount,
  ...
  document.getElementById("reviewModeSwitchRow").innerHTML = modes
    .map(
      (m) => `
...
function startWeakPractice() {
  ...
  if (pool.length === 0) {
    showToast("Không còn từ yếu nào 🎉");
    backToReviewModes();
    return;
  }
  const shuffled = pool.slice();
...
function gradeReviewWord(grade, requeueAtEnd) {
  ...
  if (grade === 1) {
    ...
    const insertAt = Math.min(
      reviewQueue.length,
      reviewIdx + 1 + Math.min(offset, reviewQueue.length - reviewIdx - 1),
    );
...
function flipReview() {
  ...
  document
    .getElementById("reviewCard")
    .classList.toggle("flipped", reviewFlipped);
  const btnIds = [
    "reviewAgainBtn",
    "reviewHardBtn",
    "reviewGoodBtn",
  ...
  btnIds.forEach(
    (id) => (document.getElementById(id).disabled = !reviewFlipped),
  );
  document.getElementById("reviewRateRow").style.display = reviewFlipped
    ? "grid"
    : "none";
...
/* --- Mode 2: Trắc nghiệm (multiple choice) ---
   Smart distractors: instead of pure random, prefer words that share the same
   part of speech (and topic) as the target — genuinely confusable options —
   and scale how "close" the distractors are to how many times the learner
   has already gotten this word right (more correct answers = harder options).
   Question direction also varies: EN→VI, VI→EN, or fill-in-the-blank from
   the example sentence — so the same word is tested from different angles. */
...
function pickSmartDistractors(target, count, field) {
  const pool = words.filter(
    (x) => x.id !== target.id && x[field] && x[field] !== target[field],
  );
  const isHardStage = (target.correctCount || 0) >= 2; // seen it right a couple times already
  const scored = pool.map((w) => ({
    w,
    score: distractorScore(w, target),
    r: Math.random(),
  }));
  scored.sort((a, b) =>
    isHardStage ? b.score - a.score || a.r - b.r : a.r - b.r,
  );
  return scored.slice(0, count).map((x) => x.w[field]);
}
let quizQuestionStart = 0,
  quizType = "en2vi";
function drawQuizReview(w, area) {
  ...
  quizType =
    roll < 0.4 ? "en2vi" : roll < 0.7 || !canContext ? "vi2en" : "context";
...
function renderStatsChart() {
  ...
  document.getElementById("barChart").innerHTML = counts
    .map(
      (c) =>
        `<div class="bar" style="height:${Math.max(4, (c / max) * 100)}%"></div>`,
    )
    .join("");
  document.getElementById("barLabel").innerHTML = days
    .map(
      (d) =>
        `<span>${chartRange <= 7 ? d.getDate() : d.getDate() % 5 === 0 ? d.getDate() : ""}</span>`,
    )
    .join("");
}
function renderGradeBar(counts) {
  ...
  bar.innerHTML = parts
    .map((p) => {
      ...
      return pct > 0
        ? `<div style="width:${pct}%;background:${p.color};"></div>`
        : "";
    })
    .join("");
...
function renderFsrsDonut(seg) {
  ...
  const stops =
    parts
      ...
      .filter(Boolean)
      .join(", ") || "var(--bg-2) 0deg 360deg";
  document.getElementById("fsrsDonut").style.background =
    `conic-gradient(${stops})`;
  document.getElementById("fsrsLegend").innerHTML = parts
    .map(
      (p) => `
...
const ACHIEVEMENT_DEFS = [
  {
    emoji: "🔥",
    name: "Học 7 ngày liên tiếp",
    check: (a) => Math.max(a.streak, a.best) >= 7,
  },
  { emoji: "📚", name: "Học 100 từ", check: (a) => a.learnedWords >= 100 },
  {
    emoji: "💯",
    name: "Accuracy > 95%",
    check: (a) => a.totalGraded >= 20 && a.accuracy > 0.95,
  },
  { emoji: "🚀", name: "30 ngày không bỏ học", check: (a) => a.best >= 30 },
  {
    emoji: "⭐",
    name: "1000 lần ôn",
    check: (a) => a.totalReviewsAllTime >= 1000,
  },
];
function renderAchievements() {
  const totalReviewsAllTime = Object.values(dailyLog).reduce(
    (s, l) => s + (l.reviews || 0),
    0,
  );
  const totalGraded = Object.values(dailyLog).reduce(
    (s, l) => s + (l.reviews || 0) + (l.learned || 0),
    0,
  );
...
function renderWeakDetailList() {
  const weak = getWeakWords();
  document.getElementById("weakReviewBtn").style.display = weak.length
    ? "block"
    : "none";
...
function renderStats() {
  ...
  document.getElementById("statBestStreakBig").textContent =
    computeBestStreak();
  ...
  document.getElementById("ovCompletion").textContent = total
  ...
  document.getElementById("perfAccuracy").textContent = totalGraded
    ? Math.round((totalCorrect / totalGraded) * 100) + "%"
    : "—";
  document.getElementById("perfForgotToday").textContent =
    (today.grades && today.grades[1]) || 0;
  const gradeTotals = { 1: 0, 2: 0, 3: 0, 4: 0 };
  Object.values(dailyLog).forEach((l) => {
    if (l.grades)
      [1, 2, 3, 4].forEach((g) => (gradeTotals[g] += l.grades[g] || 0));
  });
  ...
  const weak = getWeakWords();
  document.getElementById("weakSummaryCount").textContent = weak.length
    ? `${weak.length} từ yếu`
    : "Không có từ yếu";
  document.getElementById("weakSummarySub").textContent = weak.length
    ? `Từ yếu nhất: ${weak[0].word}`
    : "Chưa nhớ ổn định, dễ quên";
  document.getElementById("weakSummaryCard").style.opacity = weak.length
    ? "1"
    : ".55";
...
function applySettingsUI() {
  document
    .getElementById("swAutoSpeak")
    .classList.toggle("on", !!settings.autoSpeak);
  document.getElementById("valRetention").textContent =
    Math.round((settings.desiredRetention || 0.9) * 100) + "%";
  document.getElementById("valDailyGoal").textContent =
    settings.dailyNewGoal || 10;
  document
    .getElementById("swEnableFuzz")
    .classList.toggle("on", settings.enableFuzz !== false);
  document
    .getElementById("swDarkMode")
    .classList.toggle("on", settings.theme === "dark");
  ...
  [
    ...
    const el = document.getElementById(
      "sw" + key.charAt(0).toUpperCase() + key.slice(1),
    );
    if (el) el.classList.toggle("on", settings[key] !== false);
  });
}
function openCardFieldsModal() {
  document.getElementById("cardFieldsModal").classList.add("open");
}
function setTheme(t) {
  settings.theme = t;
  saveSettings();
...
function toggleDarkMode() {
  setTheme(settings.theme === "dark" ? "" : "dark");
}
...
function exportData() {
  const blob = new Blob([JSON.stringify({ words, dailyLog }, null, 2)], {
...
function submitRestore() {
  try {
    ...
    words = data.words;
    dailyLog =
      data.dailyLog && typeof data.dailyLog === "object" ? data.dailyLog : {};
    saveWords();
    saveLog();
...
function submitAddWord() {
  ...
  if (!word || !meaning) {
    showToast("Cần nhập từ và nghĩa");
    return;
  }
  const card = makeCard({
    word,
