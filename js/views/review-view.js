...
export function renderReviewTab(app) {
  ...
  // Mức 1: Mới học     -> Độ bền < 3 ngày (Lịch ôn 1 - 3 ngày)
  // Mức 2: Ngắn hạn    -> Độ bền 3 - 7 ngày (Lịch ôn 3 - 7 ngày, ~1 tuần)
  // Mức 3: Trung hạn   -> Độ bền 7 - 14 ngày (Lịch ôn 1 - 2 tuần)
  // Mức 4: Bền vững    -> Độ bền 14 - 30 ngày (Lịch ôn 2 - 4 tuần)
  // Mức 5: Ghi nhớ sâu -> Độ bền ≥ 30 ngày (Lịch ôn ≥ 30 ngày, trí nhớ dài hạn)
  const levels = [
    {
      id: 'lvl-1',
      tag: 'Mức 1',
  ...
  learnedCards.forEach(card => {
    ...
    let lvlIdx = 0;
    if (stability < 3) {
      lvlIdx = 0; // Mức 1: Mới học (< 3 ngày)
    } else if (stability < 7) {
      lvlIdx = 1; // Mức 2: Ngắn hạn (3 – 7 ngày)
    } else if (stability < 14) {
      lvlIdx = 2; // Mức 3: Trung hạn (7 – 14 ngày)
    } else if (stability < 30) {
      lvlIdx = 3; // Mức 4: Bền vững (14 – 30 ngày)
    } else {
      lvlIdx = 4; // Mức 5: Ghi nhớ sâu (≥ 30 ngày)
    }
    levels[lvlIdx].count++;
  ...
  if (dueEl) {
    ...
    if (dueCount > 25) {
      // Mức độ 3: Tồn đọng quá hạn nhiều (> 25 từ) -> Cảnh báo đỏ
      ...
      // Mức độ 2: Tồn đọng vừa (11 - 25 từ) -> Cảnh báo cam
      ...
      // Mức độ 1: Ôn tập bình thường hàng ngày (<= 10 từ) -> Màu thương hiệu Indigo thân thiện
  ...
  if (barsContainer) {
    ...
    levels.forEach((lvl, idx) => {
      ...
      // Chiều cao cột = Tỷ lệ số lượng từ của cấp độ này trên tổng số từ đã học
  ...
  // 3. Logic Nút CTA Thông Minh (Ôn tập hoặc Học từ mới)
  ...
  if (btnCta) {
    if (dueCount > 0) {
      ...
      // TRƯỜNG HỢP 2: Không có từ tới hạn -> Học từ mới (Chọn chủ đề)
      if (ctaIcon) ctaIcon.textContent = '🚀';
